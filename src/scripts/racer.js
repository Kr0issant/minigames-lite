...
const CONFIG = {
    ...
    speedMax: 15, // Lower max speed cap
...
function loop() {
    ...
    // Even Slower spawn rate: Start at ~2s gap (120 frames), cap at ~0.8s gap (50 frames)
    const spawnRate = Math.max(50, 120 - Math.floor(GAME.scoreTime / 2)); // Slower ramp up too
    ...
    // Update UI
    document.getElementById('score-value').innerText = GAME.scoreTime.toFixed(1) + 's';
    if (GAME.running) document.getElementById('status-value').innerText = "CALCULATING";
...
function startGame() {
    ...
    document.getElementById('best-score-value').innerText = GAME.bestTime.toFixed(1) + 's';
    document.getElementById('status-value').innerText = "LIVE";

    player.lane = 1;
    resize(); // Reset pos
    loop();

    // Notification removed to be cleaner
}
...
function endGame() {
    ...
    if (GAME.animationId) cancelAnimationFrame(GAME.animationId);
    playTone('crash');
    document.getElementById('status-value').innerText = "TERMINATED";
    ...
    document.getElementById('final-score').innerText = GAME.scoreTime.toFixed(2) + 's';
